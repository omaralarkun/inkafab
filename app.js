...
/* Persistencia del carrito en localStorage (no se pierde al recargar) */
function saveCart() {
  try { localStorage.setItem('inkafab_cart', JSON.stringify(cart)); } catch (e) {}
}
function loadCart() {
  try {
    const raw = localStorage.getItem('inkafab_cart');
...
function calculateQuote(mat, volRawCm, infill, qty) {
  ...
  const elecBase = costoBaseHelper(costoElectricidadJob, qty);
  const maqBase = costoBaseHelper(costoMaquinaJob, qty);
...
function costoBaseHelper(costoJob, qty) { return (Number(costoJob) || 0) / (Number(qty) || 1); }
...
function renderFaq() {
  ...
  const f = [
    ...
    { q: '¿Qué archivos puedo subir?', a: '.stl de hasta 50MB.' }
  ];
...
function handleFileUpload(file) {
  ...
  if (!['stl'].includes(ext)) { alert('Formato no soportado. Usa .stl'); return; }
  const MAX_SIZE = 50 * 1024 * 1024;
  if (file.size > MAX_SIZE) { alert('El archivo excede el límite de 50 MB.'); return; }
  // Limpiar timer de preview anterior si existe
  if (window.__stlTimer) { clearInterval(window.__stlTimer); window.__stlTimer = null; }
  file.arrayBuffer().then(async (arrayBuf) => {
    // Calcular SHA-256 del archivo para verificación posterior
    let fileHash = '';
    ...
    try {
      ...
      console.warn('No se pudo calcular hash SHA-256:', e);
    }
    try {
      ...
      lastAnalyzedFile = {
        ...
        fileHash, fileSize,
        fileObject: file,  // Guardar el objeto File original para subida posterior
...
function addCustomToCart() {
  ...
  const item = {
    ...
    fileHash: cf.fileHash || '', fileSize: cf.fileSize || 0,
    fileObject: cf.fileObject  // Guardar el objeto File para subida posterior
  };
  cart.push(item);
  saveCart();
...
function calcShipping(method, pesoKg) {
  if (method === 'recogida') return { price: 0, consultar: false };
  if (method === 'delivery_juliaca') return { price: Number(CONFIG.envioJuliaca) || 15, consultar: false };
  const fila = ENVIO.find(e => e.metodo === method && pesoKg >= e.min_kg && pesoKg <= e.max_kg);
  if (fila) return { price: Number(fila.precio), consultar: false };
  // Sin tarifa definida para este peso: indicar que debe consultarse
  return { price: 0, consultar: true };
}
...
function changeQty(id, delta) {
  ...
  if (!it) return;
  const newQty = (Number(it.qty) || 0) + (Number(delta) || 0);
  if (newQty < 1) return;
  it.qty = newQty;
  saveCart();
  updateCartUI();
...
function updateCartUI() {
      ...
      itemsEl.innerHTML = cart.map(it => `
        <div class="cart-item">
          <div class="cart-item-top">
            <div>
              <div class="cart-item-name">${it.name}${it.tipo === 'custom' && it.file ? ' <span class="stl-badge" title="Requiere adjuntar STL en WhatsApp">📎</span>' : ''}</div>
          ...
          <div class="cart-item-actions">
            <div class="qty-control">
              <button type="button" class="qty-btn" data-qty="-1" data-id="${it.id}">−</button>
              <span class="qty-val">${it.qty}</span>
              <button type="button" class="qty-btn" data-qty="1" data-id="${it.id}">+</button>
            </div>
            <button type="button" class="remove" data-remove="${it.id}">Eliminar</button>
          </div>
        </div>`).join('');
    }
  }
  ...
  const envioObj = calcShipping(metodo, peso);
  const envio = envioObj.price;
  const total = subtotal + envio;

  if (envioObj.consultar) {
    setText('shipping', 'Consultar envío');
    setText('total', 'Consultar total');
  } else {
...
async function checkoutFormSubmit(event) {
  ...
  const envioObj = calcShipping(metodo, peso);
  if (envioObj.consultar) {
    return alert('El peso del pedido supera las tarifas definidas. Por favor contacta para cotizar el envío.');
  }
  const envio = envioObj.price;
  const total = subtotal + envio;
  ...
  // Subir archivos STL a Supabase Storage
  const customItems = cart.filter(i => i.tipo === 'custom' && i.file);
  let stlUploadResults = [];
  let stlUploadErrors = [];
  
  if (customItems.length > 0) {
    // Usar Promise.all con map para mantener orden e índices correctos
    const uploadPromises = customItems.map(async (item, i) => {
      if (!item.fileObject) {
        return { success: false, index: i, file: item.file, error: 'No se encontró el archivo original (recargue la página y vuelva a analizar el STL)' };
      }
      try {
        const result = await uploadStlToSupabase(orden, item.fileObject, i, customItems.length);
        if (result && result.signedUrl) {
          return { success: true, index: i, file: item.file, signedUrl: result.signedUrl, uniqueName: result.uniqueName };
        } else {
          return { success: false, index: i, file: item.file, error: 'uploadStlToSupabase no retornó signedUrl' };
        }
      } catch (e) {
        console.error('Error subiendo STL:', e);
        const errorMsg = e.message || e.error_description || e.error || JSON.stringify(e) || 'Error de conexión/permiso';
        return { success: false, index: i, file: item.file, error: errorMsg };
      }
    });
    
    const results = await Promise.all(uploadPromises);
    
    // Procesar resultados manteniendo orden original
    results.forEach(result => {
      if (result.success) {
        stlUploadResults.push({ file: result.file, signedUrl: result.signedUrl, uniqueName: result.uniqueName });
      } else {
        stlUploadErrors.push(`${result.file}: ${result.error}`);
      }
    });
  }

  // Si hubo errores en la subida, mostrar alerta y NO continuar
  if (stlUploadErrors.length > 0) {
    const errorMsg = stlUploadErrors.join('\n');
    return alert('Error subiendo archivos STL a Supabase:\n' + errorMsg + '\n\nPor favor verifique la conexión e intente de nuevo.');
  }

  // Construir sección STL con URLs de Supabase
  let stlSection = '';
  if (customItems.length > 0) {
    const stlLines = customItems.map((item, idx) => {
      const sizeKB = (item.fileSize / 1024).toFixed(1);
      const sizeBytes = item.fileSize.toLocaleString();
      const hash = item.fileHash || 'NO_DISPONIBLE';
      const hashShort = hash.length > 8 ? hash.substring(0, 8) + '...' : hash;
      const uploadResult = stlUploadResults[idx];
      const urlLine = uploadResult ? `\nURL: ${uploadResult.signedUrl}` : '';
      return `${item.file} (${(item.fileSize / 1024).toFixed(1)} KB / ${item.fileSize.toLocaleString()} bytes)\nSHA-256: ${hashShort}${urlLine}\n${item.name} - ${item.material} - ${(item.infill*100).toFixed(0)}% infill`;
    }).join('\n\n');
    stlSection = `\n[ARCHIVOS STL SUBIDOS A SUPABASE]\n${stlLines}\n\n[IMPORTANTE] Los archivos STL han sido subidos automáticamente. Los enlaces arriba permiten descargarlos directamente.`;
  }
  ...
  const venta = {
    ...
    items: cart.map(i => ({
      ...
      stlUrl: (i.tipo === 'custom' && i.fileObject) ? (stlUploadResults.find(r => r.file === i.file) || {}).signedUrl : null
    })),
  ...
  clearCart(); closeCart();
  const modalMsg = customItems.length > 0
    ? `Pedido ${orden} registrado. STL subido a Supabase. Revisa WhatsApp para enviar constancia de pago.`
    : `Pedido ${orden} registrado. Revisa WhatsApp para enviar tu constancia de pago.`;
  openModal('modalSuccess', orden ? modalMsg : 'Pedido listo. Revisa WhatsApp.');
...
async function uploadStlToSupabase(orden, fileObject, index, totalCustom) {
  if (!supabaseClient || !fileObject || !CONFIG.supabase) {
    console.error('uploadStlToSupabase: missing supabaseClient, fileObject, or CONFIG.supabase', { supabaseClient, fileObject: !!fileObject, config: !!CONFIG.supabase });
    return null;
  }
  
  const safeName = sanitizeFileName(fileObject.name);
  const ext = safeName.split('.').pop() || 'stl';
  const baseName = safeName.replace(/\.[^.]+$/, '');
  const uniqueName = totalCustom > 1 ? `${baseName}_${index + 1}.${ext}` : safeName;
  
  const path = `modelos/pedidos/${orden}/${uniqueName}`;
  
  console.log('Uploading STL to Supabase:', { path, fileName: fileObject.name, size: fileObject.size, type: fileObject.type });
  ...
  const { data: signedUrlData, error: signError } = await supabaseClient.storage
    .from(CONFIG.supabase.bucket)
    .createSignedUrl(path, 60 * 60 * 24 * 365); // 1 año de expiración
  
  console.log('Signed URL response:', { signedUrlData, signError });
  ...
  if (!signError && signedUrlData && signedUrlData.signedUrl) {
    ...
    // Fallback a URL pública si signed URL falla
    console.warn('Signed URL failed, trying public URL fallback:', signError);
  ...
  if (!finalUrl) {
    ...
    throw new Error('Error generando URL (tanto firmada como pública fallaron)');
  }
  
  console.log(`${urlType} URL generated successfully:`, finalUrl);
  
  return { path, signedUrl: finalUrl, fileName: safeName, uniqueName, urlType };
}

/* Limpiar timer de preview STL al descargar la página */
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const cartItemsEl = gid('cartItems');
  if (cartItemsEl) cartItemsEl.addEventListener('click', e => {
    const q = e.target.closest('.qty-btn');
    if (q) { changeQty(q.dataset.id, Number(q.dataset.qty)); return; }
