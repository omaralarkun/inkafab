...
function renderMaterials() {
  ...
  grid.innerHTML = MATERIALS.map(m => `
    <div class="material-card" data-material="${m.id}">
      ...
      <div class="material-price">${formatPrice(m.precio_g)}/${m.unidad || 'g'} · ${m.densidad_gcc} g/cm³</div>
    </div>`).join('');
}
...
function renderFaq() {
  ...
  if (!el) return;
  const f = [
    { q: '¿Cómo calculan el precio?', a: 'Por el volumen del STL, la densidad del material y el relleno, más mano de obra.' },
  ...
  el.innerHTML = `
    <div class="quote-card">
      ...
      <table>
        ...
        <tr><td>Peso estimado</td><td>${q.pesoG.toFixed(2)} g</td></tr>
        <tr><td>Tiempo estimado</td><td>${q.tiempoHoras.toFixed(2)} h</td></tr>
      </table>
      <table class="quote-costs">
        <tr><td>Material</td><td>${formatPrice(q.desglose.material)}</td></tr>
        <tr><td>Electricidad</td><td>${formatPrice(q.desglose.electricidad)}</td></tr>
        <tr><td>Uso de máquina</td><td>${formatPrice(q.desglose.maquina)}</td></tr>
        <tr><td>Preparación</td><td>${formatPrice(q.desglose.preparacion)}</td></tr>
        <tr><td>Precio unidad</td><td><b>${formatPrice(q.precioUnit)}</b></td></tr>
        <tr><td>Cantidad</td><td>${cf.qty}</td></tr>
        <tr class="total"><td>TOTAL</td><td><b>${formatPrice(q.total)}</b></td></tr>
      </table>
      <p class="warranty-note">Garantía: ${calcWarranty(q.pesoG/1000)} meses (según peso).</p>
      <p class="estimate-note">Estimación previa al laminado. El peso y el tiempo reales pueden variar según altura de capa, perímetros, soportes y demás parámetros del slicer.</p>
...
function renderSTLPreview() {
  ...
  wrap.innerHTML = '<canvas id="stlPreviewCanvas" width="280" height="240"></canvas><p class="stl-preview-note">Vista previa (girando)</p>';
  ...
  const draw = () => {
    ...
    const sx = W/((maxX-minX)||1)*0.8, sy = H/((maxY-minY)||1)*0.8;
    const sc = Math.min(sx,sy);
    const cx = (minX+maxX)/2, cy = (minY+maxY)/2;
    const P = p => [W/2 + (p[0]-cx)*sc, H/2 - (p[1]-cy)*sc];
    cvs.strokeStyle = '#7aa5ff'; cvs.lineWidth = 1; cvs.beginPath();
    for (let i=0;i+2<pts.length;i+=3) {
      const a=P(pts[i]),b=P(pts[i+1]),c=P(pts[i+2]);
      cvs.moveTo(a[0],a[1]); cvs.lineTo(b[0],b[1]); cvs.lineTo(c[0],c[1]); cvs.closePath();
    }
    cvs.stroke();
  };
  draw();
...
function addCustomToCart() {
  ...
  const item = {
    ...
    material: (mat.nombre || mat.id), color: cf.color, infill: cf.infill,
    peso: q.pesoG * cf.qty / 1000, price: q.precioUnit, qty: cf.qty, file: cf.name
  };
  cart.push(item);
...
function updateCartUI() {
  ...
  const method = (document.getElementById('deliveryMethod') || {}).value || 'recogida';
  const shipping = calcShipping(method, peso);
  setText('subtotal', formatPrice(subtotal));
  setText('cartWeight', fmtKg(peso));
  setText('shipping', formatPrice(shipping));
  setText('total', formatPrice(subtotal + shipping));
}
...
async function checkoutFormSubmit(event) {
  ...
  msg += `Usted lleva:\n${listado}\n\n`;
  msg += `Peso del pedido: ${fmtKg(peso)}\n`;
