  ...
  el.innerHTML = `
    <div class="quote-card">
      ...
      <table class="quote-costs">
        <tr><td>Precio unidad (sin IGV)</td><td>${formatPrice(q.precioUnit)}</td></tr>
        <tr><td>Cantidad</td><td>${cf.qty}</td></tr>
        <tr><td>Subtotal</td><td>${formatPrice(q.subtotal)}</td></tr>
        <tr><td>IGV (${(q.igv*100).toFixed(0)}%)</td><td>${formatPrice(q.igvMonto)}</td></tr>
...
function addCustomToCart() {
  ...
  const item = {
    ...
    material: (mat.nombre || mat.id), color: cf.color, infill: cf.infill,
    peso: q.pesoG * cf.qty / 1000, price: q.totalConIgv / qty, qty: cf.qty, file: cf.name
  };
  cart.push(item);
