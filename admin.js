...
function saveProductoForm(id) {
  ...
  const price = num('pf-price');
  const stock = num('pf-stock');
  const peso = num('pf-peso');
  const resolucion = num('pf-res');

  if (!name) return alert('El nombre es obligatorio.');
  if (!Number.isFinite(price) || price <= 0) return alert('El precio debe ser un número mayor a 0.');
  if (!Number.isFinite(stock) || stock < 0) return alert('El stock debe ser un número >= 0.');
  if (!Number.isFinite(peso) || peso < 0) return alert('El peso debe ser un número >= 0.');
  if (!Number.isFinite(resolucion) || resolucion <= 0) return alert('La resolución debe ser un número > 0.');

  const obj = {
    name, type: val('pf-type'), category: val('pf-category'),
    desc: val('pf-desc'), material: val('pf-material') || 'pla',
    price, oldPrice: Number($('pf-old').value) || undefined,
    stock, peso, resolucion: resolucion || 0.2,
    image: val('pf-image'),
...
function exportProductos() {
  ...
  download('productos.json', JSON.stringify(productos, null, 2), 'application/json');
}
